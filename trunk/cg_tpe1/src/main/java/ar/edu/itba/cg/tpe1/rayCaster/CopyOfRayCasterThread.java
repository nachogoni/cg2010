 ...
 */
class CopyOfRayCasterThread extends Thread {
	 ...
	 */
	public CopyOfRayCasterThread(Scene scene, Camera camera, RayCaster rayCaster, CyclicBarrier cb) {
		this.cb = cb;
		this.scene = scene;
		this.camera = camera;
				...
				for (int j = fromY; j < toY; j++) {
					...
					for (Primitive p : scene.getList()) {
						...
						if (aux != null && (intersection == null || (intersection != null &&
							...
							intersection = aux;
							color = this.adjustColor(p.getColor(aux), aux.distance(origin));
						}
					}
	...
	Color adjustColor(Color color, double distance) {
		...
		System.out.println(color.getRed() / (float)Math.log(Math.E + distance));
		System.out.println(color.getGreen() / (float)Math.log(Math.E + distance));
		System.out.println(color.getBlue() / (float)Math.log(Math.E + distance));
		
		System.out.println(distance);
		
		System.out.flush();
		
		return new Color(color.getRed() * (1 / (float)Math.log(Math.E + distance)), 
				color.getGreen() * (1 / (float)Math.log(Math.E + distance)), 
				color.getBlue() * (1 / (float)Math.log(Math.E + distance))); 
